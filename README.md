...
NOTE: It now supports class file generated by Java 8, and uses repackaged ASM library from JRE 8 if detected, otherwise uses repackaged BCEL library from JRE. (The reason to use ASM library but not BCEL library is because the later one does not well support Java 8 without upgrading.)
